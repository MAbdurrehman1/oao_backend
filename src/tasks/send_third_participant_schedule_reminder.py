...
def _get_email_template() -> Template:
    template_path = os.path.join(
        SOURCE_DIR, "templates", "third_scheduling_delay_reminder_template.html"
    )
...
def send_participant_schedule_reminder_email(participation_id: UUID):
    ...
    MailRepository.send_mail(
        ...
        subject=f"FINAL ALERT: 3 Days Left - "
        f"Complete OAO x {organization.company_name} Training Now",
        html_str=rendered_html,
    )
    ReminderRepository.store_success(
        postfix="participant_scheduling:003:",
        identifier=participation_id,
    )


class SendThirdParticipantScheduleReminderTask(AbstractRetryTask):
    postfix = "participant_scheduling:003:"
    ...
    def main(cls, *args, **kwargs):
        ...
        participation_ids = ParticipationRepository.get_participants_before_end_date(
            before_hours=configs.third_participant_schedule_reminder_before_hours,
            status=ParticipationStatus.INVITED,
            exclude=notified_participation_ids,
            min_campaign_length_hours=configs.third_reminder_campaign_length_hours,
        )
...
@celery_app.task
def send_third_participant_schedule_reminder_task():
    SendThirdParticipantScheduleReminderTask.execute()
