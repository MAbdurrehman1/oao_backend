...
class SendSecondMissingScheduleReminderTask(AbstractRetryTask):
    postfix = "participant_missing_schedule:002:"
    ...
    def main(cls, *args, **kwargs):
        ...
        participation_ids = ModuleScheduleRepository.get_schedule_missing_participants(
            after_hours=configs.second_missing_schedule_reminder_delay_hours,
            before_hours=configs.second_missing_schedule_reminder_hand_off_hours,
...
def _get_email_template() -> Template:
    template_path = os.path.join(
        SOURCE_DIR, "templates", "second_missing_schedule_reminder_template.html"
    )
...
@celery_app.task
def send_second_missing_schedule_reminder_task():
    SendSecondMissingScheduleReminderTask.execute()
